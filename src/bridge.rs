...
pub struct SharedLevels {
    ...
    drift_db: AtomicU32,     // f32 bits, baseline drift since calibration; NaN when uncalibrated
}
...
impl Default for SharedLevels {
    ...
    /// silent rather than a full-width red bar. The threshold and drift have
    /// no honest silent value, so they start NaN; readers already guard a
    /// non-finite threshold/drift as "not available yet".
    fn default() -> Self {
        Self {
            level_db: AtomicU32::new((-100.0f32).to_bits()),
            threshold_db: AtomicU32::new(f32::NAN.to_bits()),
            peak_db: AtomicU32::new((-100.0f32).to_bits()),
            drift_db: AtomicU32::new(f32::NAN.to_bits()),
        }
    }
}

impl SharedLevels {
    pub fn store(&self, level_db: f32, threshold_db: f32, peak_db: f32, drift_db: f32) {
        ...
        self.drift_db.store(drift_db.to_bits(), Ordering::Relaxed);
    }
    ...
    pub fn load(&self) -> (f32, f32, f32, f32) {
        (
            ...
            f32::from_bits(self.drift_db.load(Ordering::Relaxed)),
        )
    }
}
...
mod tests {
    ...
    fn shared_levels_round_trip() {
        ...
        s.store(-42.5, -30.0, -12.25, 1.5);
        assert_eq!(s.load(), (-42.5, -30.0, -12.25, 1.5));
    }

    #[test]
    fn shared_levels_default_reads_as_silent_with_no_threshold_or_drift() {
        // Before the audio thread's first store, a fresh load must not read
        // as 0.0 dBFS (full scale) — that would paint a full red bar.
        let s = SharedLevels::default();
        let (level_db, threshold_db, peak_db, drift_db) = s.load();
        assert_eq!(level_db, -100.0);
        assert!(threshold_db.is_nan());
        assert_eq!(peak_db, -100.0);
        assert!(drift_db.is_nan());
    }
}
